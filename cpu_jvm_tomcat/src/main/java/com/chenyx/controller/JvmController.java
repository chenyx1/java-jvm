...
public class JvmController {
    ...
    @GetMapping("/cpu_gc/{a}")
    public Object cpu_gc(@PathVariable("a") Integer a) {
        int count = 1;
        List<JvmController> jvmControllers = new ArrayList<>();
        while (true) {
            if (a == 1) {
                break;
            }
            JvmController jvmController = new JvmController();
            jvmControllers.add(jvmController);
            System.out.println("count:" + count + ",a=" + a);
        }
        return  "返回cpu_gc成功！";
    }
    ...
    public Object cpu_lock(@PathVariable("a") Integer a) throws Exception{
        ...
        for (int i = 0; i < a ; i++) {
            if (i % 2 == 0) {
                WorkTask workTask = new WorkTask(num1,num2);
                Future<Object> future =  executorService.submit(workTask);
                futures.add(future);
            } else {
                WorkTask workTask = new WorkTask(num2,num1);
                Future<Object> future =  executorService.submit(workTask);
                futures.add(future);
            }
        }
